...
log('extra macs = %s GMac' % (model_restoration.flops(x) / 1e9), train_log)
...
for epoch in range(start_epoch, opt['TRAIN']['TOTAL_EPOCHS'] + 1):
    ...
    for iter, data in enumerate(train_loader, 1):
        ...
        # restored_fft = torch.stack([restored_fft.real, restored_fft.imag], dim=-1)
        ...
        if iter % opt['TRAIN']['PRINT_FRE'] == 0:
            log("Epoch:%-4d \tIter:%-4d \tTime:%-4.f \tL1_loss:%.6f \tFFT_loss:%.6f \t"%(epoch, iter, time.time()-epoch_start_time, loss_L1, loss_fft*0.1),train_log)
    ...
    if epoch >= opt['TRAIN']['VAL']:
        if epoch % opt['TRAIN']['VAL_FRE'] == 0:
            ...
            for ii, data_val in enumerate(tqdm(val_loader), 1):
                ...
                psnr_val_rgb.append(PSNR(restored_img, target))
